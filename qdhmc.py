"""File containing the QDHMC integration with TFP."""
import sys
...
class QDHMCKernel(tfp.python.mcmc.kernel.TransitionKernel):
    ...
    def generate_circuit(self, r, xs, eta, lam):
        ...
        Returns:
            - (cirq.Circuit): the trotterized circuit
        """
        ...
        for _ in range(r):
            ...
            # Convert between position and momentum space
...
if __name__ == "__main__":
    ...
    def log_prob(x):
        #return np.sum([-1 * x[i] for i in range(len(x))])
        #return -1 * x[0] - x[1] - x[2] - x[0]**2 - x[1]**2 - x[2]**2
        #return (x[0] + x[1] + x[2])/-3
        #return np.sum([2 * x[i] for i in range(n)])
        return -1 * x[0] - x[0]**2
    ...
    #print(tf.cast(tf.reshape(domain_float("".join([str(bit) for bit in [random.randint(0,1) for _ in range(pres)]])), [1, 1]), tf.float64))
    #tk = QDHMCKernel(log_prob, pres, t, r, 2)
    #print("RESU:T", tk.one_step(tf.convert_to_tensor([1.4, -0.4]), RWResult(target_log_prob=0.0, log_acceptance_correction=0.0)))

    test_c = HMC(log_prob, num_vars=n, precision=pres)
    test_q = HMC(log_prob, kernel_type='quantum', precision=pres, r=r, num_vars=n, t=t)

    #ins = np.random.randint(0, 2, size=(1, s)) * 2 - 1
    #old_res = test.kernel.bootstrap_results(ins)
    #print(test.kernel.one_step(current_state=ins, previous_kernel_results=old_res))
    s, m, std, a, rs = test_c.run_hmc(100, 0)
    print(s[-1])
    print('mean:{:.4f}  stddev:{:.4f}  acceptance:{:.4f}'.format(m.numpy(), std.numpy(), a.numpy()))
    plt.plot(-rs[0], label='c')
    #tf.random.set_seed(32)
    #print(tf.random.uniform(shape=[2], minval=-200.0, maxval=200.0, seed=42))
    s, m, std, a, rs = test_q.run_hmc(100, 0)#, init_state=tf.random.uniform(shape=[2], minval=-200.0, maxval=200.0, seed=42))
    print(s[-1])
    print('mean:{:.4f}  stddev:{:.4f}  acceptance:{:.4f}'.format(m.numpy(), std.numpy(), a.numpy()))
    plt.plot(-rs[0], label='q')
    plt.legend()
    plt.show()
